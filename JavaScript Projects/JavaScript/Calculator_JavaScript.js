...
const Calculator = {
    ...
    First_Operand: null,
    // this checks whether or nor the second operand has been input
...
function Input_Decimal(dot) {
    ...
    // doesn't couse bugs in your operation
    if (Calculator.Wait_Second_Operand === true) return;
    if (!Calculator.Display_Value.includes(dot)) {
    // we are saying that if the Display_Value does not contain a decimal point
    // we want to add a decimial point
        Calculator.Display_Value += dot;
    }
}

//This seciton handles operators
function Handle_Operator(Next_Operator) {
    const { First_Operand, Display_Value, operator } = Calculator
    //when an operator key is pressed, we convert to current number
...
Update_Display();
// thsi  section monitors button clicks
